...
const showEmployeeInfo = 'SELECT employee.id, employee.first_name, employee.last_name,  roles.title, roles.salary, department.name as department concat (manager.first_name, " ", manager.last_name) as manager FROM employees as employee JOIN roles on employee.role_id = roles.id JOIN department ON roles.department_id = department.id LEFT JOIN employees as manager ON employee.manager_id = manager.id;';
...
const showEmployees = 'SELECT  manager.id, concat (manager.first_name, " ", manager.last_name) as manager, employee.id, concat (employee.first_name, employee.last_name) AS employee FROM employees as employee JOIN employees as manager ON employee.manager_id = manager.id ORDER BY manager;';
...
// Delete function
const deleteRow = (table, id, value) =>`DELETE FROM ${table} WHERE  ${id}= ${value};`;
...
module.exports = { showDepartments, showRoles, showEmployeeInfo, updateManager, updateRole, showEmployees, showEmployeesbyDepartment, deleteRow,showDepartmentBudget }